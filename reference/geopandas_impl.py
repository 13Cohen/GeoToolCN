...
from GeoToolCN._hierarchy import MERGED_PREFIXES as _MERGED_PREFIXES
from GeoToolCN._hierarchy import parent_city_code
...
_DEFAULT_DATA_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "GeoToolCN", "data"
)
...
class GeoTool:
    ...
    def _point_in_level(self, level: str, point: Point) -> Region | None:
        ...
        # Some district polygons overlap, so a point can have several valid
        # candidates.  SPEC §3.4 breaks the tie by ascending adcode; taking the
        # R-tree's own order instead makes the answer depend on GEOS internals
        # and therefore liable to shift with a library upgrade.
        candidates.sort(key=lambda idx: str(gdf.iloc[idx]["adcode"]))
    ...
    def is_in_china(self, lat: float, lng: float) -> bool:
        ...
        bool
        """
        # SPEC §2.8: equivalent to reverse() resolving a province.  Testing the
        # province polygon directly disagrees with reverse() on islands, which
        # the district layer covers but the province outline does not.
        return self.reverse(lat, lng).province is not None
