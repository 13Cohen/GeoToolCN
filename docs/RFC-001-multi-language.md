...
|------|------|---------|---------|---------|
| `mini` | META + NAMES，无几何 | ~150 KB | ~60 KB | 级联选择器、adcode 查询、名称搜索、坐标转换。浏览器首选 |
| `lite` | + 1e-4 几何（约 10 m）+ 0.1° 网格 | ~3.5 MB | ~1.6 MB | 绝大多数业务反查 |
| `full` | + 1e-5 几何（约 1 m）+ 0.05° 网格 | **5.8 MB** | **~2.6 MB** | 需要边界精度的场景（默认） |

> `full` 的 5.8 MB 为**实测值**（见 §11）：几何 3.76 MB + 网格索引 2.06 MB。
...
| 数据文件 | 28 MB GeoJSON | **5.8 MB** `.gtc` | 4.8x |
...
**选择 B**：npm / PyPI 的包分发本身就带压缩传输（5.8 MB → 约 2.6 MB 上线体积），
