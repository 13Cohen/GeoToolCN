...
- **差分对拍进 CI**（每次 PR，20 万均匀点 + 每区县 3 个边界点）。它是唯一独立于「黄金集
  由之生成的那个实现」的检查，此前只能手动运行，RFC-002 写的 Nightly 并不存在。
- **DIV-104 真正算距离。** `differential.py` 以前把任何区县不一致都归入 DIV-104，不看点到
  边界的距离——把网格候选顺序反过来、所有重叠带整体翻转，也报"未登记差异 0"。现在从
  `known-divergences.yaml` 读 `bound.quantisation_steps`，距争议边界超过一步即为未登记差异；
  同一变异现在报 46 条。实测 21,640 个边界样本全部在 0.45 步内。
- `validate_gtc.py`：索引与源几何不一致的点同样要求在一步之内（以前只有 0.5% 的比率阈值，
  影响 0.4% 面积的错误能过）；实现 RFC-002 L0 #6 的代表点漂移检查（常量定义了从未用过）。
- `run.py --allow` 中一次都没匹配到的差异 id 视为过期登记而失败；DIV-103 匹配器改为只豁免
  自洽的答案。
- `test.yml` 不再硬编码 `record_count == 3271`，改读 `DATA_VERSION.json`。
- dev 依赖加上界（`geopandas<2`、`shapely<3`、`numpy<3`）：构建管线走 GEOS，主版本升级应是
  有意的"重建并重生成"事件。
